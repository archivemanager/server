...
import org.archivemanager.util.URLUtility;
...
public class FastStringBuffer implements Comparable, CharSequence, CharSpan {
	 ...
	 */
	public int hashCode() {
		return hashCodeForCharSpan(this);
	}
	...
	public void xmlEncode(int offset, int length) {
		...
		//if (extraSpace == 0)
		//	return; // no encoding necessary  @todo restore this
...
/*            } else if (ch < 0x0E && ch != 0x9 && ch != 0xA && ch != 0xD) {
                // example &#x07;
                 * this doesn't work; xml still doesn't like control chars
                array[outPtr--] = ';';
                array[outPtr--] = hexchars[ch & 0xF];
                array[outPtr--] = hexchars[(ch >>> 4) & 0xF];
                array[outPtr--] = 'x';
                array[outPtr--] = '#';
                array[outPtr--] = '&';
*/
	 ...
	 */
	public void appendWithURLEncode(String str) {
		URLUtility.urlEncode(str, this);
	}
	 ...
	 */
	public void appendWithJSONEncode(String str) {
		int len = str.length();
		for (int i = 0; i < len; i++) {
